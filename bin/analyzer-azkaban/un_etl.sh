...
hdfs_input=$(GetConf "hdfs.un_in")/${log_type}/${day}/${date}
...
#执行ETL主程序
${HADOOP_HOME}/bin/hadoop jar ${main_jar}  com.sinova.dw.etl.service.EtlUnSmsService 0 ${sys_conf} ${etl_conf} ${hdfs_input} ${hdfs_output} ${log_type}-${date} ${run_type}
h_tab_name="t_ods_"${log_type}
etl_count=`${HADOOP_HOME}/bin/hadoop fs -text ${hive_warehouse}/${h_tab_name}/${dt}/*${date}* | wc -l`
#azkaban任务调度监控点
...
#加载错误日志进错误表分区
err_count=0
err_count=`${HADOOP_HOME}/bin/hadoop fs -text ${hdfs_output}/err* | wc -l`
if [ $err_count -gt 0 ]
then
