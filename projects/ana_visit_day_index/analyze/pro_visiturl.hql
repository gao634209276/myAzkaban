CREATE TEMPORARY FUNCTION str_to_date AS 'com.nexr.platform.hive.udf.UDFStrToDate';
CREATE TEMPORARY FUNCTION decode AS 'com.nexr.platform.hive.udf.GenericUDFDecode';
set hive.auto.convert.join=false;
-- 频道/站点 每个受访页面指标  t_ana_visiturl  t_ana_visiturl_all
insert overwrite table t_ana_visiturl
  select total.visit_site,
         total.visit_chid,
         total.visit_url,
          substr(str_to_date('${dayAgo}',  "yyyyMMdd"), 0, 10),
         total.pv,
         total.uv,
         total.ip,
         jumpstay.staycnt,
         jumpstay.sessioncnt
    from (select visit_site,
                 visit_chid,
                 visit_url,
                 count(1) pv,
                 count(distinct visit_id) uv,
                 count(distinct visit_ip) ip
            from t_uba_log
           where dt = '${dayAgo}'
             and visit_site is not null
             and visit_chid is not null
             and visit_url is not null
           group by visit_site, visit_chid, visit_url) total
    left join (select sum(jump) jumpcnt,
                      round(sum(stay)) staycnt,
                      sum(scn) sessioncnt,
                      visit_site,
                      visit_chid,
                      visit_url
                 from (select visit_site,
                              visit_chid,
                              count(distinct session_id) scn,
                              visit_url,
                              decode(count(1), 1, 1, 0) jump,
                              max(to_unix_timestamp(visit_date)) -
                              min(to_unix_timestamp(visit_date)) stay
                         from t_uba_log
                        where dt = '${dayAgo}'
                        group by visit_site,
                                 visit_chid,
                                 visit_url) tt
                group by tt.visit_site, tt.visit_chid, tt.visit_url) jumpstay on total.visit_site =
                                                                                 jumpstay.visit_site
                                                                             and total.visit_chid =
                                                                                 jumpstay.visit_chid
                                                                             and total.visit_url =
                                                                                 jumpstay.visit_url;

-- 站点 每个受访页面指标    t_ana_visiturl_all
insert overwrite table t_ana_visiturl_all
  select total.visit_site,
         total.visit_url,
      substr(str_to_date('${dayAgo}',  "yyyyMMdd"), 0, 10),
         total.pv,
         total.uv,
         total.ip,
         jumpstay.staycnt,
         jumpstay.sessioncnt
    from (select visit_site,
                 visit_url,
                  count(1) pv,
                 count(distinct visit_id) uv,
                 count(distinct visit_ip) ip
            from t_uba_log
           where dt = '${dayAgo}'
             and visit_site is not null
               and visit_url is not null
           group by visit_site, visit_url) total
    left join (select sum(jump) jumpcnt,
                       sum(stay) staycnt,
                      sum(scn) sessioncnt,
                      visit_site,
                        visit_url
                 from (select visit_site,
                                 count(distinct session_id) scn,
                              visit_url,
                              decode(count(1), 1, 1, 0) jump,
                              max(to_unix_timestamp(visit_date)) -
                              min(to_unix_timestamp(visit_date)) stay
                         from t_uba_log
                        where dt = '${dayAgo}'
                        group by visit_site,
                                   visit_url ) tt
                group by tt.visit_site,  tt.visit_url) jumpstay on total.visit_site =
                                                                                 jumpstay.visit_site
                                                                              and total.visit_url =
                                                                                 jumpstay.visit_url;                                                                                 