...
-- 频道/站点 老访客指标  t_ana_ouser,t_ana_ouser_all 
insert overwrite table t_ana_ouser
  select total.visit_site,
    ...
    from (select t1.visit_site,
           ...
           where t1.dt = '${dayAgo}'
             and t2.cnt > 1
           group by t1.visit_site, t1.visit_chid) total
    left join (select tt.visit_site,
                      visit_chid,
                      sum(jump) jumpcnt,
                 ...
                 from (select t1.visit_site,
                         ...
                         from t_uba_log t1
                         join tmp_uba_visit_sid t2 on t1.visit_id =
                                                      t2.visit_id
                                                       and t1.visit_site=t2.visit_site
                        where t1.dt = '${dayAgo}'
                          and t2.cnt > 1
...
-----------------------------------------------------------------------------------------------
-- 频道/站点 老访客入口页面PV t_ana_ouser_in ,t_ana_ouser_in_all
insert overwrite table t_ana_ouser_in
...
-----------------------------------------------------------------------------------------------                                                 
 -- 频道/站点 老访客来源UV  t_ana_ouser_source  t_ana_ouser_source_all
insert overwrite table t_ana_ouser_source
