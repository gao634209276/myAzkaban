...
insert overwrite table t_ana_ouser_all
...
insert overwrite table t_ana_ouser_in_all
select t3.visit_site,
        substr(str_to_date('${dayAgo}',  "yyyyMMdd"), 0, 10),
       t3.refer_url,
        count(t3.visit_url) pv
  from (select visit_site, visit_chid, session_id, min(to_unix_timestamp(visit_date)) mindate, dt
          from t_uba_log
...
insert overwrite table t_ana_ouser_source_all
