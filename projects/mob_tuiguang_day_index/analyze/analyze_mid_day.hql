...
insert overwrite table t_tmp_tg_level partition
  (tg_level = 'C')
...
insert overwrite table t_tmp_tg_level partition(tg_level = 'D')
select t1.mob, t1.dt_log, '登录', '', ''
  from (select mob, max(dt_log) dt_log
          from t_tmp_f_mob_mon
         where dt_log >=tg_6m and dt_log<dt_tg
         group by mob) t1
  left join (select mob from t_tmp_tg_level where tg_level = 'C') t2 on t1.mob =
                                                                        t2.mob
 where t2.mob is null;
...
insert overwrite table t_tmp_tg_level partition(tg_level='A')
select  mob,dt_log,'登录','','' from t_tmp_f_mob_A;
...
insert overwrite table t_tmp_tg_level partition
 ...
 insert overwrite table t_tmp_tg_level partition
  (tg_level = '3D')
  select t1.mob, t2.order_time, '合约类商品订购', '', ''
    from (select mob from t_tmp_tg_level where tg_level = 'D') t1
    join t_ods_scmsg_tg t2 on t1.mob = t2.user_mobile;
   
	
 
 insert into table t_tmp_tg_level partition
   (tg_level = '3A')
...
--办理业务 剔除流量包用户
 insert overwrite table t_tmp_tg_level partition(tg_level='2A')
  select t1.user_mobile, t1.query_time, '办理业务', '', ''
    from (select user_mobile, query_time
            from t_mid_tg_f_mob_intf) t1
    join t_tmp_f_mob_A t2 on t2.mob = t1.user_mobile
  left join(
    select distinct mob
      from t_tmp_tg_level
     where tg_level = '3A'
 ...
 insert overwrite table t_tmp_tg_level partition(tg_level='2D')
  select t1.user_mobile, t1.query_time, '办理业务', '', ''
    from (select user_mobile, query_time
            from t_mid_tg_f_mob_intf ) t1
    join (select mob from t_tmp_tg_level where tg_level = 'D') t2 on t2.mob = t1.user_mobile
  left join(
    select distinct mob
      from t_tmp_tg_level
     where tg_level = '3D'
...
insert into table t_tmp_tg_level partition(tg_level='2A')
...
insert into table t_tmp_tg_level partition(tg_level='2D')
select t1.mob, t2.dt, '交费充值','',''
  from (select mob from t_tmp_tg_level where tg_level = 'D') t1
  join (select user_mobile mob, payment_time dt
          from t_ods_jfmsg_tg) t2 on t1.mob = t2.mob;